...
for nimage in range(len(train_X_letter)):
    features.append(train_X_letter[nimage]) 
    containers.append(train_Y_letter[nimage]+9) # starts category indexing after mnist

for nimage in range(len(test_X_letter)):
    features.append(test_X_letter[nimage])
    containers.append(test_Y_letter[nimage]+9)

print(np.unique(containers))
